...
struct lean_metrics_snapshot
{
    ...
    uint64_t peer_connection_events_total[LEAN_METRICS_DIR_COUNT]
                                         [LEAN_METRICS_CONN_RESULT_COUNT];
    uint64_t
        peer_disconnection_events_total[LEAN_METRICS_DIR_COUNT]
                                       [LEAN_METRICS_DISCONNECT_REASON_COUNT];
    uint64_t
        aggregator_skipped_total[LEAN_METRICS_AGGREGATOR_SKIPPED_REASON_COUNT];
    ...
    struct lean_metrics_histogram_snapshot
        pq_sig_aggregated_signatures_building_time;
    struct lean_metrics_histogram_snapshot
        pq_sig_aggregated_signatures_verification_time;
    struct lean_metrics_histogram_snapshot
        pq_sig_block_aggregated_signatures_verification_time;
    struct lean_metrics_histogram_snapshot
        committee_signatures_aggregation_time;
    struct lean_metrics_histogram_snapshot
        block_build_stage_vote_collection_time;
    struct lean_metrics_histogram_snapshot
        block_build_stage_key_sig_deserialize_time;
    ...
    struct lean_metrics_histogram_snapshot
        block_build_stage_other_prover_setup_time;
...
void lean_metrics_record_block_building_payload_aggregation_time(
    double seconds);
...
void lean_metrics_record_state_transition_slots(uint64_t slots_processed,
                                                double seconds);
void lean_metrics_record_state_transition_block(double seconds);
void lean_metrics_record_state_transition_attestations(uint64_t count,
                                                       double seconds);
...
void lean_metrics_record_pq_aggregated_signature_build(size_t attestation_count,
                                                       double seconds);
void lean_metrics_record_pq_aggregated_signature_verification(double seconds,
                                                              bool valid);
void lean_metrics_record_pq_block_aggregated_signatures_verification(
    double seconds);
void lean_metrics_record_committee_signature_aggregation(
    double seconds, uint64_t aggregated_attestations);
