...
int lantern_xmss_load_secret_file(const char *path,
                                  struct PQSignatureSchemeSecretKey **out_key);
int lantern_xmss_load_public_file(const char *path,
                                  struct PQSignatureSchemePublicKey **out_key);
int lantern_xmss_load_secret_bytes(const uint8_t *data, size_t length,
                                   struct PQSignatureSchemeSecretKey **out_key);
int lantern_xmss_load_public_bytes(const uint8_t *data, size_t length,
                                   struct PQSignatureSchemePublicKey **out_key);
