...
int lantern_signed_block_list_resize(LanternSignedBlockList *resp,
                                     size_t new_length);

int lantern_network_status_encode(const LanternStatusMessage *status,
                                  uint8_t *out, size_t out_len,
                                  size_t *written);
int lantern_network_status_decode(LanternStatusMessage *status,
                                  const uint8_t *data, size_t data_len);

int lantern_network_blocks_by_root_request_encode(
    const LanternBlocksByRootRequest *req, uint8_t *out, size_t out_len,
    size_t *written);
int lantern_network_blocks_by_root_request_decode(
    LanternBlocksByRootRequest *req, const uint8_t *data, size_t data_len);

int lantern_network_blocks_by_range_request_decode(
    LanternBlocksByRangeRequest *req, const uint8_t *data, size_t data_len);
int lantern_network_blocks_by_range_request_encode(
    const LanternBlocksByRangeRequest *req, uint8_t *out, size_t out_len,
    size_t *written);
