...
struct lantern_http_server_callbacks
{
    ...
    int (*snapshot_fork_choice)(
        void *context, struct lantern_fork_choice_tree_snapshot *out_snapshot);
    int (*metrics_snapshot)(void *context,
                            struct lantern_metrics_snapshot *out_snapshot);
    int (*finalized_state_ssz)(void *context, uint8_t **out_bytes,
                               size_t *out_len);
    int (*finalized_block_ssz)(void *context, uint8_t **out_bytes,
                               size_t *out_len);
...
int lantern_http_server_start(struct lantern_http_server *server,
