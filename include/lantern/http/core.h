...
typedef int (*lantern_http_route_handler)(
    void *context, const struct lantern_http_request *request);
...
int lantern_http_core_start(struct lantern_http_core_server *server,
                            const struct lantern_http_core_config *config);
void lantern_http_core_stop(struct lantern_http_core_server *server);

int lantern_http_send_response(const struct lantern_http_request *request,
                               int status_code, const char *status_text,
                               const char *content_type, const char *body,
                               size_t body_len);
int lantern_http_send_json_error(const struct lantern_http_request *request,
                                 int status_code, const char *status_text,
