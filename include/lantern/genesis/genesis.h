...
void lantern_genesis_artifacts_init(
    struct lantern_genesis_artifacts *artifacts);
void lantern_genesis_artifacts_reset(
    struct lantern_genesis_artifacts *artifacts);
int lantern_genesis_load(struct lantern_genesis_artifacts *artifacts,
                         const struct lantern_genesis_paths *paths);
struct lantern_validator_config_entry *
lantern_validator_config_find(struct lantern_validator_config *config,
                              const char *name);
int lantern_validator_config_assign_ranges(
    struct lantern_validator_config *config, uint64_t validator_count);
int lantern_validator_config_apply_assignments(
    struct lantern_validator_config *config, const char *path,
    uint64_t validator_count);
