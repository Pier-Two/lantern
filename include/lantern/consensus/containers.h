...
#define LANTERN_SIGNATURE_SIZE 1208
#endif
...
#define LANTERN_VALIDATOR_PUBKEY_SIZE 32
...
#define LANTERN_JUSTIFICATION_VALIDATORS_LIMIT                                 \
    ((size_t)LANTERN_HISTORICAL_ROOTS_LIMIT *                                  \
     (size_t)LANTERN_VALIDATOR_REGISTRY_LIMIT)
...
int lantern_validator_index_compute_subnet_id(LanternValidatorIndex index,
...
void lantern_aggregated_attestation_init(
    LanternAggregatedAttestation *attestation);
void lantern_aggregated_attestation_reset(
    LanternAggregatedAttestation *attestation);
int lantern_aggregated_attestation_copy(
    LanternAggregatedAttestation *dst, const LanternAggregatedAttestation *src);
...
int lantern_aggregated_attestations_resize(LanternAggregatedAttestations *list,
                                           size_t new_length);

void lantern_aggregated_signature_proof_init(
    LanternAggregatedSignatureProof *proof);
void lantern_aggregated_signature_proof_reset(
    LanternAggregatedSignatureProof *proof);
int lantern_aggregated_signature_proof_copy(
    LanternAggregatedSignatureProof *dst,
    const LanternAggregatedSignatureProof *src);

void lantern_signed_aggregated_attestation_init(
    LanternSignedAggregatedAttestation *attestation);
void lantern_signed_aggregated_attestation_reset(
    LanternSignedAggregatedAttestation *attestation);
