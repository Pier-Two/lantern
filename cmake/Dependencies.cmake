...
function(lantern_configure_dependencies target)
    ...
    if(EXISTS ${libp2p_source_dir}/CMakeLists.txt)
        if(NOT TARGET libp2p_unified)
            ...
            # Mirror the list of libp2p tests into CTestCustom so top-level
            # `ctest` runs ignore dependency-only suites instead of failing
            # because the corresponding executables are never built.
            file(GLOB_RECURSE _lantern_libp2p_ctest_files
                 "${CMAKE_BINARY_DIR}/c-libp2p/CTestTestfile.cmake"
                 "${CMAKE_BINARY_DIR}/c-libp2p/*/CTestTestfile.cmake")
            set(_lantern_libp2p_tests_to_ignore "")
            foreach(_lantern_ctest_file IN LISTS _lantern_libp2p_ctest_files)
                file(STRINGS "${_lantern_ctest_file}" _lantern_ctest_lines REGEX "^add_test")
                foreach(_lantern_ctest_line IN LISTS _lantern_ctest_lines)
                    if(_lantern_ctest_line MATCHES "add_test\\(\\[=\\[(.+)\\]=\\]")
                        list(APPEND _lantern_libp2p_tests_to_ignore "${CMAKE_MATCH_1}")
                    elseif(_lantern_ctest_line MATCHES "add_test\\(([^ ]+)")
                        list(APPEND _lantern_libp2p_tests_to_ignore "${CMAKE_MATCH_1}")
                    endif()
                endforeach()
            endforeach()
            list(REMOVE_DUPLICATES _lantern_libp2p_tests_to_ignore)
            set(_lantern_ctest_custom "${CMAKE_BINARY_DIR}/CTestCustom.cmake")
            if(_lantern_libp2p_tests_to_ignore)
                set(_lantern_ignore_block "")
                foreach(_lantern_test_name IN LISTS _lantern_libp2p_tests_to_ignore)
                    string(APPEND _lantern_ignore_block "    ${_lantern_test_name}\n")
                endforeach()
                file(CONFIGURE OUTPUT "${_lantern_ctest_custom}" CONTENT
"# Auto-generated by lantern/cmake/Dependencies.cmake to suppress c-libp2p tests.
set(CTEST_CUSTOM_TESTS_IGNORE
${_lantern_ignore_block})
" @ONLY)
            elseif(EXISTS "${_lantern_ctest_custom}")
                file(REMOVE "${_lantern_ctest_custom}")
            endif()
