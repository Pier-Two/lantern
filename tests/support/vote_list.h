...
static inline int lantern_attestations_resize(LanternAttestations *list,
                                              size_t length)
{
    ...
    if (length > list->length)
    {
        memset(list->data + list->length, 0,
               (length - list->length) * sizeof(*list->data));
    }
    else if (length < list->length)
    {
        memset(list->data + length, 0,
               (list->length - length) * sizeof(*list->data));
    }
    list->length = length;
    return 0;
}
