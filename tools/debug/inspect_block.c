...
int main(int argc, char **argv) {
    ...
    if (lantern_hash_tree_root_block_body(&block.message.body, &body_root) != 0) {
    ...
    format_root(&block.message.parent_root, parent_hex, sizeof(parent_hex));
    format_root(&block.message.state_root, state_hex, sizeof(state_hex));
